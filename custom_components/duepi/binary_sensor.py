...
from .coordinator import DuepiCoordinator, state_has_problem
...
async def async_setup_entry(
    ...
    async_add_entities(
        [
            DuepiOnlineBinarySensor(coordinator, device_id),
            DuepiProblemBinarySensor(coordinator, device_id),
        ]
    )
...
class DuepiProblemBinarySensor(CoordinatorEntity[DuepiCoordinator], BinarySensorEntity):
    """Binary sensor indicating whether the stove reports a problem."""
    ...
    @property
    def is_on(self) -> bool | None:
        """Return True if the stove reports an alarm or error status."""
        return state_has_problem(self.coordinator.data)
